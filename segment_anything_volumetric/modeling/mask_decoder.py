...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0) # 最开始先随机初始化的iou_token和mask_tokens
        ...
        src = src + dense_prompt_embeddings  # 稀疏提示 加上 图像映射
        pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)  # 扩展维度
        b, c, h, w, d = src.shape

        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens)  # src为图片em+稀疏em  tokens 为两个
        iou_token_out = hs[:, 0, :]
        ...
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w * d)).view(b, -1, h, w, d)  # 点乘操作
