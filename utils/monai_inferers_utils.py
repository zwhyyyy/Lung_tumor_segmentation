...
def logits2roi_coor(spatial_size, logits_global_single):
    # crop predict
    pred_global_single = torch.sigmoid(logits_global_single) > 0.5  # 就是把缩小得到的预测区域得到  进行裁剪
    ## get all pos idx
    nonzero_indices = torch.nonzero(pred_global_single)  # 多少行（多少个非0的数） 三列（h, w, d)
...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    compute_dtype = inputs.dtype  # inputs 就是裁剪后的尺寸
    ...
    roi_size = fall_back_tuple(roi_size, image_size_)  # roi_size（32，256，256） image_size_（31，255，253）  确保图像大小兼容
    # in case that image size is smaller than roi size
    image_size = tuple(max(image_size_[i], roi_size[i]) for i in range(num_spatial_dims))
    pad_size = []
    for k in range(len(inputs.shape) - 1, 1, -1):  # range(start, stop, step)
...
def generate_box(pred_pre, bbox_shift=None): # 这里传进来的是resize的label值
    meaning_post_label = pred_pre  # （32，256，256）[h, w, d]  d标识维度
    ones_idx = (meaning_post_label > 0).nonzero(as_tuple=True)  # (x_idxs, y_idxs, z_idxs) 返回一个三维元组  即在三维度上标识这个物体
    ...
    if bbox_shift is None:
        ...
        for coor in min_coords:
            coor_ = max(0, coor)  # 将0和coor作比较 谁大取谁
...
def select_points(preds, num_positive_extra=4, num_negative_extra=0, fix_extra_point_num=None):
    ...
    if all(tmp.nelement() == 0 for tmp in ones_idx):  # 如果全是负样本
        # all neg
    ...
    if num_positive_extra > 0:  # 如果额外需要正样本
    ...
    if num_negative_extra > 0:  # 如果额外需要负样本
